...
### Задание 6
Поместил общие определение ресурсов в общий блок:
```
...
```
Почему изменений быть не должно, если в одном случае было значение core_fraction=5, а в другом core_fraction=20 ? :)
Результат применения:
```
Terraform will perform the following actions:
