...
Вместо настройки вручную я определил нужные для создания backend ресурсы с помощью Terraform в файле `backend.tf`:
```
#=== Service account
...
Пример создания таблицы с помощью AWS CLI:`
```
aws dynamodb create-table \
  --table-name terraform_locks \
  --attribute-definitions AttributeName=LockID,AttributeType=S \
...
Добрый день уважаемые проверяющие преподаватели!

Прошу вас проверить мое решение до уже скорого технического закрытия (вечером 24 ноября) этого модуля.

Отправил линк на пока ещё несуществующую страницу на всякий случай заранее, чтобы успеть до закрытия модуля.

Сейчас пока раннее утро 24 ноября, я доделываю работу. Выполненная работа должна появиться через 3-4 часа, т.е. сутра 24 ноября уже будет доступна для проверки.

Пожалуйста, проверьте эту работу после появления соответстующего файла на Github.com (когда исчезнет сообщение "404 - page not found").


https://github.com/a-prokopyev-resume/devops-netology/blob/main/07-terraform-new/05/hw-05.md
